/**
 * Toast Notifications - Investment Activity Alerts
 * Shows random investment notifications from different countries
...
(function() {
    ...
    const countryList = [
        ...
        "Nepal", "Netherlands (the)", "New Caledonia", "New Zealand", "Nicaragua", "Niger (the)",
        "Nigeria", "Niue", "Norfolk Island", "Northern Mariana Islands (the)", "Norway", "Oman",
    ...
    function initToastNotifications() {
        // Check if toast element exists and jQuery is available
        if (typeof $ === 'undefined' || !$('#toast').length) {
            return;
        }
        ...
        // Show random investment notification
        function showToast() {
            const random = Math.floor(Math.random() * countryList.length);
            const selectedCountry = countryList[random];

            const randomamt = Math.floor(Math.random() * amounts.length);
            const selectedAmt = amounts[randomamt];

            const generatedText = "Someone from " + selectedCountry + " just Invested <b class='text-info'>$" + selectedAmt + "</b> in RSFX";

            $('#toast').html(generatedText);
            $('#toast').toast('show');
        }
